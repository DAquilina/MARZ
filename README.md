...
The client side application uses node version 17, node 16 should also work as well. From the root of the `webapp` directory install the node dependencies using the command below

```Bash
npm ci
```

*Developer note: one of the dependencies explicitly does not support node 17*
...
To run the pytests from the root directory run the following commands while the target container is running
...
1. Develop `/ProductsPage` (React)
2. Develop products API
3. Add valid product images to initial data
